...
This interview targets a **waterfall / ISO 29148-style** requirements specification — the resulting document will be a flat list of numbered functional requirements (FR-NNN) and non-functional requirements (NFR-XXX-NNN), each phrased as "the system shall …". Do **not** ask the operator to decompose the work into agile features or user stories — that vocabulary belongs to the agile path. Keep the questions framed around system capabilities and "shall" statements.
...
- Enumerate the distinct system capabilities. For each capability, state what the system shall do, who triggers it, and the observable outcome.
- For every functional requirement: at least one verifiable acceptance criterion ("the system shall return HTTP 201 when …"), the priority (Must / Should / Could), and the operational owner.
- Explicit non-goals — what is intentionally out of scope and why.
- Avoid agile vocabulary (no "as a … I want … so that …" framing; no INVEST checklist). State requirements as imperatives.
...
- Adversarial scenarios: describe each misuse case in terms of an attacker's goal and the system's countermeasure (no "as a malicious actor I want …" agile framing — state the threat and the control directly).
...

{
  "modules": [
    ...
    {"name": "FUNCTIONAL REQUIREMENTS", "questions": [...]},
